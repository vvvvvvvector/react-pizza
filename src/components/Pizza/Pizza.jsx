...
export const Pizza = ({ onClickImage, id, types, sizes, name, cost, imageURL, weight, diameter, description }) => {
    ...
    const currentPizza = useSelector((state) => state.cart.pizzas.find((obj) => obj.name === name && obj.type === types[selectedType] && obj.diameter === diameter[selectedSize]));
    const amount = currentPizza ? currentPizza.amount : 0;

    const pizzaObj = { types, id, sizes, name, cost, imageURL, weight, diameter, description };

    const onClickAdd = () => {
        dispatch(addPizza({
            id: id,
            type: types[selectedType],
            diameter: diameter[selectedSize],
            name: name,
            amount: 1,
            cost: cost,
            imageURL: imageURL
